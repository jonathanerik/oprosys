			...
			<table>
				...
				<?php
					...
					if ($username == 'superior1')
					{
						...
						if ( $resultif == 0)
						{
							echo '<tr><td colspan = 5><h1>THERE ARE NO APPROVED REQUEST FORMS</h1></td></tr>';
						}
						else
						{
						...
						else
						{
							while ( $record = mysql_fetch_assoc($result))
							{
								echo '<tr>';
									...
									echo '<td>'.$record['status'].'</td>';
									echo "<td class = 'action'><button type = 'button' class =  'details'><a href =\"view_approve1.php?formid={$record['formid']}\">VIEW DETAILS</a></button></td>";
									echo "<td class = 'action'><button type = 'button' class =  'details'><a href =\"view_approve1.php?formid={$record['formid']}\">GENERATE</a></button></td>";
								echo '</tr>';
							}
						}
					}
